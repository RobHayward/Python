...
```
# Methods
This is the same as a function but it is called on a specific value. For example, `index` is a method that will find the index of a particular list.  However, you have to call that method on the oject `listname.index`.  Each data type has its own set of methods. 
...
A list can be seen as a single character string.  Many of the functrions can be used.  While strings can be index but they are immutable (cannot be changed). To change strings, there is a need to create a new string.

Immutable objects (like lists) stand alone and the list themselves just contain references to the lsit. THerefore, if the list is assigned to another name and a change in made in the list under that name, the list referred to by the original name will also change. Therefore, if a list is used and changed inside a function, this will also change the global value.   Immutable variables like strings and duples do not face this problem. 
...
list(eggs.items())
```
It is usually best to test whether key exists before tryuing to return its value. this will prevent an error messagte. 
...
A multiline string can also be made with three (double) quotations. For example, 
...
isspace()| Space only
istitle()| titecase only
.title()| Changes something to title
join()| Will join strings together (join can be specificed)
.split()| Splits a string
ljust()| left justified (length of string and filling can be spefified)
